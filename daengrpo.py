...
# code to run Danish GRPO with English support. Models needs to be changed according to which models/adapters are used. 
...
PROMPT_MODE = "da_en"   # options: "da", "da_en"
...
def build_prompt_da_en(da_question: str, en_question: str, allow_reasoning: bool) -> str:
    if allow_reasoning:
        return (
        ...
        return (
            "Du får både en dansk opgave og en engelsk referenceoversættelse.\n"
            "Brug kun den engelske tekst til at afklare betydningen af problemet.\n\n"
            f"DA: {da_question}\n\n"
            f"EN: {en_question}\n\n"
            "Returnér KUN det endelige svar på én linje PRÆCIS i dette format:\n"
...
def make_run_name() -> str:
    ...
    return (
        f"grpo_{prompt_tag}_{model_tag}_{adapter_tag}_{mode_tag}_"
...
def main():
    ...
    print(f"Using {len(ds)} rows for GRPO training ({PROMPT_MODE}).")
    ...
    def to_grpo(ex):
        ...
        if PROMPT_MODE == "da":
            prompt_text = build_prompt_da(da_q, ALLOW_REASONING)
        elif PROMPT_MODE == "da_en":
            prompt_text = build_prompt_da_en(da_q, en_q, ALLOW_REASONING)
        else:
    ...
    print(f"Starting GRPO with PROMPT_MODE={PROMPT_MODE} ...")
    trainer.train()
